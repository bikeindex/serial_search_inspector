...
RSpec.describe UpdateLastRequestAtJob, type: :job do
  describe 'perform' do
    context 'serial searched before' do
      ...
      let(:serial_search) { FactoryBot.create(:serial_search, last_request_at: old_time) }
      let(:ip_address) { FactoryBot.create(:ip_address, last_request_at: old_time) }
      let!(:log_line) { FactoryBot.create(:log_line, request_at: target_time, serial_search: serial_search, ip_address: ip_address) }
      let!(:log_line_2) { FactoryBot.create(:log_line, request_at: old_time, serial_search: serial_search, ip_address: ip_address) }
